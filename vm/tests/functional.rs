...
use solcery_vm::{
    ...
    vmcommand::{CommandByteCode, VMCommand},
    word::Word,
    word_vec,
};
...
async fn initialize_dummy() {
    let card_types = generate_testing_card_types();
    let board = generate_initial_board();
    let instruction_rom = generate_instruction_rom();
...
async fn test(
    instruction_rom: Vec<VMCommand>,
    card_types: Vec<CardType>,
    instruction_bytes: Vec<u8>,
    board: Board,
    expected_board: Board,
) {
    // TODO: Adequate variable names!!!
    let program_id = Pubkey::new_unique();
    let card_types_id = Pubkey::new_unique();
    let instruction_rom_id = Pubkey::new_unique();
    ...
    let card_types = card_types.try_to_vec().unwrap();
    let mut card_types_account = AccountSharedData::new(1_000, 1024, &engine_id);
    card_types_account.set_data(card_types);

    let instruction_rom: Vec<CommandByteCode> = instruction_rom
        .into_iter()
        .map(CommandByteCode::from)
        .collect();
    let instruction_rom: &[u8] = cast_slice(instruction_rom.as_slice());
    let mut instruction_rom_account = AccountSharedData::new(1_000, 1024, &engine_id);
    instruction_rom_account.set_data_from_slice(instruction_rom);

    let mut board = board.try_to_vec().unwrap();
    board.extend(std::iter::repeat(0).take(8192 - board.len()));
    ...
    program.add_account(card_types_id, Account::from(card_types_account));
    program.add_account(board_id, Account::from(board_account));
    program.add_account(instruction_rom_id, Account::from(instruction_rom_account));
    ...
    let mut transaction = Transaction::new_with_payer(
        &[Instruction::new_with_bytes(
            ...
            vec![
                ...
                AccountMeta::new_readonly(card_types_id, false),
                AccountMeta::new_readonly(instruction_rom_id, false),
...
fn generate_testing_card_types() -> Vec<CardType> {
...
fn generate_initial_board() -> Board {
    unsafe { Board::from_raw_parts(vec![], word_vec![], 0) }
}
fn generate_instruction_rom() -> Vec<VMCommand> {
    vec![
