use super::*;
use crate::board::{MemoryRegion, PlayerBoard};
...
fn testing_board() -> PlayerBoard {
    ...
    let common_region = MemoryRegion::with_data(vec![card1, card2, card3, card4], board_attrs);

    unsafe { PlayerBoard::from_raw_parts(vec![Some(common_region)], 5, 1) }
}

fn initial_board() -> PlayerBoard {
    let card1 = type1().instantiate_card(1);
    let board_attrs = word_vec![0, 0, 0, false, false, false,];
    let common_region = MemoryRegion::with_data(vec![card1], board_attrs);
    unsafe { PlayerBoard::from_raw_parts(vec![Some(common_region)], 1, 1) }
}
...
fn pop_attr() {
    ...
    let region = board_needed.memory_region(0).unwrap();
    region.cards[1].attrs[3] = Word::Numeric(42);
